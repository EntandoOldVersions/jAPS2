...
<p><s:text name="note.deleteContentModel.areYouSure" />&#32;<em class="important"><span class="monospace"><s:property value="modelId" /></span> &ndash; <s:property value="description" /> (<s:property value="%{getSmallContentType(getContentModel(modelId).contentType).descr}" />)</em>? <wpsf:submit action="delete" value="%{getText('label.remove')}" cssClass="button" /></p>
</s:form>
</div>