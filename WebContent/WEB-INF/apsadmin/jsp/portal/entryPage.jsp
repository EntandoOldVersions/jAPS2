...
<p>
	...
	<wpsf:textfield name="pageCode" id="pageCode" disabled="%{getStrutsAction() == 2}" cssClass="text" />
</p>
...
<p>
	...
	<wpsf:select name="group" id="group" list="allowedGroups" listKey="name" listValue="descr" disabled="%{groupSelectLock}" cssClass="text"></wpsf:select>
...
<s:iterator value="extraGroups" id="groupName">
	<li>
		...
		<wpsf:submit action="%{#actionName}" type="image" src="%{#removeIcon}" value="%{getText('label.remove')}" title="%{getText('label.remove')}" />: <s:property value="%{getSystemGroups()[#groupName].getDescr()}"/> 
	</li>
</s:iterator>
</ul>
</s:if>
<p>
	...
	<wpsf:submit action="joinExtraGroup" value="%{getText('label.join')}" cssClass="button" />
</p>
...
<p>
	...
	<wpsf:select name="model" id="model" list="pageModels" listKey="code" listValue="descr" cssClass="text"></wpsf:select>
</p>
