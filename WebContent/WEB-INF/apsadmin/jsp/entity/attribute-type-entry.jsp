...
<fieldset><legend><s:text name="label.info" /></legend>
	<p>
		...
		<wpsf:textfield cssClass="text" name="attributeTypeCode" value="%{attributeTypeCode}" disabled="true" />
	</p>
	...
	<s:else>
		<wpsf:textfield cssClass="text" name="attributeName" value="%{attributeName}" id="attributeName" disabled="true" />
	</s:else>	
	</p>

	<p>
		<wpsf:checkbox name="required" id="required" cssClass="radiocheck" /><label for="required"><s:text name="Entity.attribute.flag.mandatory.full" /></label>
	</p>
...
<s:if test="!(#attribute.type == 'List') && !(#attribute.type == 'Monolist')">
	...
	<p>
		<wpsf:checkbox name="searcheable" id="searcheable" cssClass="radiocheck" /><label for="searcheable"><s:text name="Entity.attribute.flag.searcheable.full" /></label>
	</p>
	</s:if>
</s:if>
</fieldset>
...
<fieldset><legend><s:text name="name.roles" /></legend>
	...
	<s:iterator value="attributeRoles" id="attributeRole">
		...
		<tr>
			...
			<td><wpsf:submit action="%{#actionName}" type="image" src="%{#removeIcon}" value="%{getText('label.remove')}" title="%{getText('label.remove')}" /></td>
		</tr>
	</s:iterator>
	</table>
	</s:if>
